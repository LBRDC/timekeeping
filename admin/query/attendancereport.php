...
function getUser($con, $id, $start_date, $end_date)
{
    ...
    $userDetails = ["user" => ['idnumber' => '', 'emp_name' => '', 'emp_status' => '', 'department' => '', 'position' => '', 'location' => ''], 'attendance' => []];

    $sql = "SELECT emp.idnumber, concat(emp.lastname,' ',emp.firstname) as emp_name, emp.employment_type as emp_status, fd.name as department, fp.name as position, fl.name_location as location  FROM employees as emp inner join field_department as fd on fd.fld_dept_id = emp.department inner join field_position as fp on fp.fld_position_id = emp.position inner join field_location as fl on fl.fld_location_id = emp.location WHERE idnumber = $id";
