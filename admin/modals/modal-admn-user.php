<!-- MODAL ADD ACCOUNT -->
<div class="modal fade" id="addEmployee_user" tabindex="-1" role="dialog" aria-labelledby="mobile_addEmployee" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"> <span class="text-success font-weight-bold">Add User</span></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="addAccountUser">
                <div class="modal-body">
                    <div class="mdl-importfile">
                        <div class="form-row mb-2">
                            <label for="fld_location">Employee<span class="text-danger">*</span></label>
                            <select class="form-control custom-select-location" name="useraccount_emp" id="useraccount_emp">
                                <option value="" selected disabled>Select Employee...</option>
                                <?php foreach ($employees as $list) : ?>
                                    <option value="<?= $list['idnumber'] ?>" data-pos="<?= $list['position'] ?>" data-name="<?= $list['name'] ?>" data-posid="<?= $list['positionid'] ?>">[<?= $list['idnumber'] ?>] <?= $list['name'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <input type="text" name="useraccount_name" id="useraccount_name" hidden>
                        <input type="text" name="useraccount_posid" id="useraccount_posid" hidden>
                        <div class="form-row mb-2">
                            <label for="startingRow">Position<span class="text-danger">*</span></label>
                            <input type="text" name="useraccount_pos" id="useraccount_pos" class="form-control" placeholder=""
                                autocomplete="off" required style="width:100%" readonly>
                        </div>
                        <div class="form-row mb-2">
                            <label for="startingRow">Username<span class="text-danger">*</span></label>
                            <input type="text" name="useraccount_user" id="useraccount_user" class="form-control" placeholder=""
