...
$emptbl_query = "select e.IdNumber, concat(e.LastName, ', ', e.FirstName, ' ', e.MiddleName) as name, l.name_location, m.accountID, m.Email, m.Password, m.Status from mobile_account as m INNER JOIN employee_tbl as e on e.IdNumber = m.Employee INNER JOIN field_location as l on l.fld_location_id = m.Location";
...
<!-- &&& EMPLOYEE MANAGE &&& -->  
                  ...
                  <table class="table align-items-center table-flush table-hover" id="mobileUserTable">
                    <thead class="thead-light">
                        ...
                        foreach ($employees_tbl as $emp) {
                          $pw = $emp['Password'] == 'LBRDC' ? '<span class="badge badge-secondary p-2">DEFAULT</span>' : '<span class="badge badge-success p-2">UPDATED</span>';
  ...
  <div class="btn-group btn-group-sm" role="group" aria-label="First group">
    <button type="button" class="btn btn btn-warning acc_edit" title="Edit"  data-toggle="modal" data-target="#mobile_editEmployee" data-id="' . $emp['accountID'] . '"> <i class="fas fa-edit"></i></button>
